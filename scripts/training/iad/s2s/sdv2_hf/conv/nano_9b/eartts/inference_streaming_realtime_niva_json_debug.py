...
class RealtimeStreamingInference:
    ...
    def _update_audio_buffer(self, audio_buffer, buffer_fill_level, new_audio, buffer_size_samples, accumulated_audio=None):
        """
        ...
        Parameters:
            ...
            accumulated_audio (torch.Tensor): Tensor holding all audio samples accumulated so far.
        ...
        Returns:
            Tuple[torch.Tensor, int, torch.Tensor, torch.Tensor]:
                ...
                - `accumulated_audio`, all audio samples accumulated so far.
        ...
        Notes:
            ...
            is not yet full.
        """
        # Initialize accumulated_audio if None
        if accumulated_audio is None:
            accumulated_audio = torch.empty(1, 0, dtype=new_audio.dtype, device=new_audio.device)
        
        # Append new audio to accumulated audio
        if new_audio.shape[1] > 0:
            accumulated_audio = torch.cat([accumulated_audio, new_audio], dim=1)
        
        if new_audio.shape[1] == 0:
            current_buffer = audio_buffer[:, :buffer_fill_level]
            return audio_buffer, buffer_fill_level, current_buffer, accumulated_audio
    ...
    def infer_one_step(self,
        ...
        # do "perception" step outside the for-loop - use accumulated_audio instead of audio_input
        # Pass all accumulated audio to perception
        accumulated_audio_len = torch.tensor([accumulated_audio.shape[1]], dtype=torch.long, device=self.device)

        source_encoded_all, _, _ = self.model.stt_model.perception(
            input_signal=accumulated_audio,
            input_signal_length=accumulated_audio_len,
            return_encoder_emb=True,
        )

        source_encoded_all = source_encoded_all.to(self.dtype)
        
        # Extract a window of frames that corresponds to the audio_input size
        # Convert audio_input samples to frames (each frame = 80ms = FRAME_SIZE_SAMPLES samples)
        window_size = int(audio_input.shape[1] / FRAME_SIZE_SAMPLES)
        window_size = max(1, window_size)  # Ensure at least 1 frame
        
        # Extract the last window_size frames from source_encoded_all
        source_encoded = source_encoded_all
        total_encoded_frames = source_encoded.shape[1]

        # logging.info(f"frame_idx: {frame_idx}, accumulated_audio shape: {accumulated_audio.shape}, source_encoded shape: {source_encoded.shape}")
    ...
    def inference_realtime_streaming(self, audio_path: str, num_frames_per_inference: int = None, explore_padding: bool = True, request_id: Optional[str] = None):
        ...
        accumulated_audio = None  # Initialize accumulated audio
        ...
        while frame_idx < total_frames:
            ...
            audio_buffer, buffer_fill_level, current_buffer, accumulated_audio = self._update_audio_buffer(
                audio_buffer, buffer_fill_level, new_audio, buffer_size_samples, accumulated_audio
            )
    ...
    try:
        ...
        try:
            for idx, record in enumerate(input_records):
                ...
                # Compute WER between predicted ASR and ground truth
                pred_asr_text = results['asr_text'][0] if 'asr_text' in results else ''
                wer_score = None
                if ground_truth_text and pred_asr_text:
                    try:
                        wer_score = jiwer.wer(ground_truth_text, pred_asr_text)
                        # Print WER in green color using ANSI escape codes
                        print(f"\033[92m📊 WER: {wer_score:.4f} ({wer_score*100:.2f}%)\033[0m")
                        logging.info(f"   Ground truth: {ground_truth_text}")
                        logging.info(f"   Predicted ASR: {pred_asr_text}")
                    except Exception as e:
                        logging.warning(f"⚠️  Could not compute WER: {e}")
                        wer_score = None
        ...
        # Compute average WER
        wer_scores = [rec['wer'] for rec in output_records if rec.get('wer') is not None]
        if wer_scores:
            avg_wer = sum(wer_scores) / len(wer_scores)
            print(f"\033[92m📊 Average WER across {len(wer_scores)} utterances: {avg_wer:.4f} ({avg_wer*100:.2f}%)\033[0m")
