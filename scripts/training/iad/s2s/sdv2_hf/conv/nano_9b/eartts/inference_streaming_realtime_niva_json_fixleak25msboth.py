...
# Extra lookahead buffer beyond 1 frame
EXTRA_BUFFER_MS = 0.0125  # 12.5ms
EXTRA_BUFFER_SAMPLES = int(SAMPLE_RATE * EXTRA_BUFFER_MS)  # 400 samples
...
DEFAULT_INITIAL_BUFFER_FRAMES = 1  # [DEPRECATED] Not used anymore - buffer is calculated from num_frames_per_inference
...
class RealtimeStreamingInference:
    ...
    def _update_audio_buffer(self, audio_buffer, buffer_fill_level, new_audio, buffer_size_samples, left_overlap_samples=0, right_overlap_samples=0, frame_idx=0):
        """
        ...
        Parameters:
            ...
            buffer_fill_level (int): Number of valid samples currently stored in `audio_buffer` (counting only the window part, not overlaps).
            new_audio (torch.Tensor): Incoming samples of shape `[1, slice_n_samples]` for the current step.
            buffer_size_samples (int): Total capacity of the buffer in samples (includes left and right overlaps).
            left_overlap_samples (int): Number of samples from the previous window to keep on the left side.
            right_overlap_samples (int): Number of samples to keep on the right side for lookahead.
                                        This handles the case where we buffer more samples than we advance.

        Returns:
            Tuple[torch.Tensor, int, torch.Tensor]:
                - Updated `audio_buffer` containing the newest samples (always capped to `buffer_size_samples`).
                - Updated `buffer_fill_level`, reflecting how many contiguous samples are valid (window only).
                - `current_buffer`, a view over the valid portion of the buffer used for the model input.

        Notes:
            Buffer structure: (left_overlap_samples, window, right_overlap_samples)
        ...
        remaining = new_audio
        window_size = buffer_size_samples - left_overlap_samples - right_overlap_samples

        # Warm-up phase: buffer doesn't have left overlap yet
        if left_overlap_samples + buffer_fill_level + right_overlap_samples < buffer_size_samples and remaining.shape[1] > 0:
            ...
            if warmup_take > 0:
                ...
                buffer_fill_level += warmup_take - right_overlap_samples
                remaining = remaining[:, warmup_take:]

        # Buffer is full, slide the window and keep overlaps
        if remaining.shape[1] > 0:
            if remaining.shape[1] >= buffer_size_samples:
                audio_buffer = remaining[:, -buffer_size_samples:]
            else:
                # Now remaining should roughly has shape: window_size + right_overlap_samples
                # Slide the buffer: take the last (left_overlap_samples + remaining.shape[1] - right_overlap_samples) samples from the old buffer,
                # then append the new audio
                # Keep the last left_overlap_samples from the previous window
                if frame_idx == DEFAULT_BUFFER_SIZE_FRAMES:
                    shift_amount = remaining.shape[1] - right_overlap_samples - left_overlap_samples
                    right_shift_amount = right_overlap_samples*2
                else:
                    shift_amount = remaining.shape[1] - right_overlap_samples
                    right_shift_amount = right_overlap_samples
                # import pdb; pdb.set_trace()
                audio_buffer = torch.cat([
                    audio_buffer[:, shift_amount: -right_shift_amount],
                    remaining
                ], dim=1)
            buffer_fill_level = window_size

        current_buffer = audio_buffer if buffer_fill_level + left_overlap_samples + right_overlap_samples == buffer_size_samples else audio_buffer[:, :buffer_fill_level + right_overlap_samples]

        # logging.info(f'current_buffer size: {current_buffer.shape[1]}')
        
        return audio_buffer, buffer_fill_level, current_buffer

    def _write_debug_info(self, frame_idx, audio_input, source_encoded, max_frames=3):
        """
        Save audio_input and source_encoded tensors for debugging purposes.
        
        Parameters:
            frame_idx (int): Current frame index.
            audio_input (torch.Tensor): Input audio tensor.
            source_encoded (torch.Tensor): Encoded source tensor from perception.
            max_frames (int): Maximum number of frames to save (default: 3).
        """
        if frame_idx < max_frames:
            ...
            # Extract utterance file name (without extension) from audio path
            if hasattr(self, 'current_audio_path') and self.current_audio_path:
                utterance_name = os.path.splitext(os.path.basename(self.current_audio_path))[0]
            else:
                utterance_name = "unknown"
            
            # Create file name with utterance name and frame index
            debug_filename = f"realtime_{utterance_name}_frame{frame_idx}.pt"
            debug_path = os.path.join(debug_dir, debug_filename)
            
            # Create debug data for this specific frame
            debug_data = {
                'frame_idx': frame_idx,
                'audio_input': audio_input.cpu(),
                'source_encoded': source_encoded.cpu(),
                'utterance': utterance_name
            }
            
            # Save to file
            torch.save(debug_data, debug_path)
            print(f"Saved frame {frame_idx} to {debug_path}")
    ...
    def infer_one_step(self,
        ...
        # Extract utterance name for debug logging
        import os
        if hasattr(self, 'current_audio_path') and self.current_audio_path:
            utterance_name = os.path.splitext(os.path.basename(self.current_audio_path))[0]
        else:
        ...
        # Debug: Save audio_input and source_encoded for first three frames
        # self._write_debug_info(frame_idx, audio_input, source_encoded)
        
        total_encoded_frames = source_encoded.shape[1]
        
        # In 25ms lookahead mode:
        # - audio_input is 1 frame + 25ms (105ms = 1680 samples)
        # - source_encoded will have ~2 frames after perception
        # - We take the FIRST frame (embedding_position = 0) for decoding
    ...
    @torch.no_grad()
    def inference_realtime_streaming(self, audio_path: str, num_frames_per_inference: int = None, explore_padding: bool = False, request_id: Optional[str] = None):
        """
        Perform realtime streaming inference simulating microphone capture with 25ms lookahead.
        
        This mode uses 1 frame (80ms) + 25ms = 105ms buffer. After perception processes this,
        it outputs ~2 frames, but we only use the FIRST frame for decoding.
        
        Args:
            audio_path: Path to input audio file (simulates microphone input)
            num_frames_per_inference: Number of frames to process per inference step (must be 1)
            explore_padding: [DEPRECATED] Not used in 25ms lookahead mode
            
        Returns:
            Dictionary with 'text', 'tokens_text', 'tokens_audio', 'audio', 'audio_len'
        """
        # Store audio path for debug purposes
        ...
        buffer_size_samples = buffer_size_frames * FRAME_SIZE_SAMPLES + EXTRA_BUFFER_SAMPLES + EXTRA_BUFFER_SAMPLES  # left + window + right
        
        # Validation
        if num_frames_per_inference != 1:
            raise ValueError(
                f"num_frames_per_inference must be 1 for this 25ms lookahead mode (got {num_frames_per_inference})."
            )
        
        # Calculate buffer samples using num_frames_per_inference
        buffer_samples_per_step = FRAME_SIZE_SAMPLES * num_frames_per_inference + EXTRA_BUFFER_SAMPLES + EXTRA_BUFFER_SAMPLES  # left + window + right
        
        logging.info(f"Buffer size: 1 frame + 25ms ({buffer_size_samples} samples = {buffer_size_samples/SAMPLE_RATE:.3f}s)")
        logging.info(f"Buffer per step: {num_frames_per_inference} frame + 25ms ({buffer_samples_per_step} samples)")
        ...
        if explore_padding:
            ...
            embedding_position = -2  # Default to second-last position
        ...
        # Calculate total frames to decode
        total_frames = int(np.ceil(total_samples / FRAME_SIZE_SAMPLES))
        
        # Calculate required samples: buffer 25ms more on every step
        # Example: to decode 100 frames with 1 frame + 25ms per step:
        #   - Step 0: read 1 frame + 25ms (1680 samples) → decode frame 0
        #   - Step 1: read 1 frame + 25ms (1680 samples) → decode frame 1
        #   - ...
        #   - Step 99: read 1 frame + 25ms (1680 samples) → decode frame 99
        # Total samples needed: buffer_samples_per_step * total_frames
        required_total_samples = buffer_samples_per_step * total_frames
        
        if required_total_samples > total_samples:
            audio_signal = np.pad(audio_signal, (0, required_total_samples - total_samples), mode='constant')
            logging.info(f"   Padded to: {required_total_samples} samples")
        
        num_inference_steps = total_frames
        logging.info(f"   Frames to decode: {total_frames}")
        logging.info(f"   Buffer per step: {num_frames_per_inference} frame + 25ms ({buffer_samples_per_step} samples)")
        ...
        # audio_sample_idx tracks which samples we're reading from the input audio
        frame_idx = 0
        audio_sample_idx = 0
        
        while frame_idx < total_frames:
            # logging.info(f'frame_idx: {frame_idx}')
            # Buffer num_frames_per_inference frames + 25ms on every iteration
            # slice_n_samples = buffer_samples_per_step  # 1 frame + 25ms = 1680 samples
            
            slice_start = audio_sample_idx
            slice_n_samples = buffer_samples_per_step - EXTRA_BUFFER_SAMPLES  # Only buffer the right overlap

            slice_end = slice_start + slice_n_samples
            new_audio = audio_signal_tensor[:, slice_start:slice_end]
            
            # Remove the overlapping portion (EXTRA_BUFFER_SAMPLES on both sides) before adding new audio
            audio_buffer, buffer_fill_level, current_buffer = self._update_audio_buffer(
                ...
                left_overlap_samples=EXTRA_BUFFER_SAMPLES,
                right_overlap_samples=EXTRA_BUFFER_SAMPLES,
                frame_idx=frame_idx
            )
            
            # Always decode num_frames_per_inference frames (typically 1)
            ...
            audio_sample_idx += num_frames_per_inference * FRAME_SIZE_SAMPLES
...
def main():
    ...
    parser.add_argument("--input_json", type=str, required=True,
                       help="Path to input JSON file containing list of records with audio_path and text fields")
    ...
    parser.add_argument("--buffer_size_frames", type=int, default=70,
                       help="[DEPRECATED in 25ms mode] Size of audio buffer in frames (each frame = 80ms)")
    parser.add_argument("--initial_buffer_frames", type=int, default=DEFAULT_INITIAL_BUFFER_FRAMES,
                       help="[DEPRECATED] Not used - buffer is now calculated from num_frames_per_inference")
    parser.add_argument("--num_frames_per_inference", type=int, default=DEFAULT_NUM_FRAMES_PER_INFERENCE,
                       help="Number of frames per inference step (must be 1 for 25ms lookahead mode)")
    ...
    parser.add_argument("--combine_inp_out_audio", action="store_true",
                    help="Whether to decode audio")
    parser.add_argument("--extra_buffer_ms", type=float, default=0.0125,
                       help="Extra buffer in milliseconds for lookahead (default: 0.0125 = 12.5ms)")
    ...
    # Update global buffer constants from command-line argument
    global EXTRA_BUFFER_MS, EXTRA_BUFFER_SAMPLES
    EXTRA_BUFFER_MS = args.extra_buffer_ms
    EXTRA_BUFFER_SAMPLES = int(SAMPLE_RATE * EXTRA_BUFFER_MS)
        ...
        try:
            for idx, record in enumerate(input_records):
                ...
                results = model.inference_realtime_streaming(
                    ...
                    explore_padding=(args.explore_padding and idx == 0),  # Only explore padding on first record
                )
                ...
                try:
                    if ground_truth_text.strip() and pred_asr_text.strip():
                        utterance_wer = wer(ground_truth_text, pred_asr_text)
                ...
                output_record = {
                    ...
                    'src_text': ground_truth_text,  # Ground truth
                    'pred_src_text': pred_asr_text,  # Predicted ASR
                    'pred_text': results['text'][0],  # Predicted agent text
                    'wer': utterance_wer,  # WER score
                }
                
                # Save audio if available
                if args.decode_audio and 'audio' in results and results['audio'] is not None:
                    audio_filename = f"audio_{idx:04d}.wav"
                    audio_path_out = os.path.join(args.output_dir, audio_filename)
                    
                    audio_np = results['audio'].float().cpu().numpy()
                    
                    import soundfile as sf
                    sf.write(audio_path_out, audio_np, model.target_sample_rate)
                    output_record['output_audio_path'] = audio_path_out
                    logging.info(f"✅ Audio saved: {audio_path_out}")
                    
                    # Save stereo audio (input + output) if requested
                    if args.combine_inp_out_audio and 'input_audio' in results:
                        stereo_filename = f"audio_{idx:04d}_stereo.wav"
                        ...
                        # Align lengths
                        target_len = min(len(input_audio_resampled), len(output_audio))
                        ...
                        sf.write(stereo_path_out, stereo_audio, output_sr)
                        output_record['output_stereo_audio_path'] = stereo_path_out
