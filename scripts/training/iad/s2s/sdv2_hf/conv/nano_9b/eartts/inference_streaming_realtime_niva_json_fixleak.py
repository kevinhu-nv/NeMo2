...
DEFAULT_INITIAL_BUFFER_FRAMES = 2  # Number of frames to buffer initially before first inference
...
class RealtimeStreamingInference:
    ...
    def inference_realtime_streaming(self, audio_path: str, num_frames_per_inference: int = None, explore_padding: bool = True, request_id: Optional[str] = None):
        ...
        initial_buffer_frames = int(self.model_cfg.get("initial_buffer_frames", DEFAULT_INITIAL_BUFFER_FRAMES))
        ...
        if initial_buffer_frames < 1:
            raise ValueError(f"initial_buffer_frames ({initial_buffer_frames}) must be at least 1.")
        if initial_buffer_frames > buffer_size_frames:
            raise ValueError(
                f"initial_buffer_frames ({initial_buffer_frames}) must be "
                f"less than or equal to buffer_size_frames ({buffer_size_frames})."
            )
        
        logging.info(f"Buffer size: {buffer_size_frames} frames ({buffer_size_frames * FRAME_SIZE_SEC}s)")
        logging.info(f"Initial buffer: {initial_buffer_frames} frames")
        ...
        # Calculate required samples: initial buffer + remaining frames - 1
        # Example: to decode 100 frames with initial_buffer=2:
        #   - Step 0: read frames 0,1 → decode frame 0
        #   - Step 1: read frame 2 → decode frame 1
        #   - ...
        #   - Step 99: read frame 100 → decode frame 99
        # Total frames needed in audio: 101
        required_audio_frames = initial_buffer_frames + total_frames - 1
        padded_total_samples = required_audio_frames * FRAME_SIZE_SAMPLES
        
        if padded_total_samples > total_samples:
            audio_signal = np.pad(audio_signal, (0, padded_total_samples - total_samples), mode='constant')
            logging.info(f"   Padded to: {padded_total_samples} samples ({required_audio_frames} frames)")
        
        num_inference_steps = total_frames
        logging.info(f"   Frames to decode: {total_frames}")
        logging.info(f"   Initial buffer: {initial_buffer_frames} frames, then {num_frames_per_inference} frame(s) per step")
        ...
        while frame_idx < total_frames:
            # On first iteration, buffer initial_buffer_frames, then 1 frame per iteration
            if frame_idx == 0:
                frames_to_add = initial_buffer_frames
            else:
                frames_to_add = num_frames_per_inference
            
            slice_start = audio_sample_idx
            slice_n_samples = frames_to_add * FRAME_SIZE_SAMPLES
...
def main():
    ...
    parser.add_argument("--initial_buffer_frames", type=int, default=DEFAULT_INITIAL_BUFFER_FRAMES,
                       help="Number of frames to buffer initially before first inference (default: 2)")
    ...
    try:
        ...
        model_cfg_dict = {
            ...
            "initial_buffer_frames": args.initial_buffer_frames,
