...
class RealtimeStreamingInference:
    ...
    @torch.no_grad()
    def inference_realtime_streaming(self, audio_path: str, num_frames_per_inference: int = None, explore_padding: bool = True):
        """
        ...
        audio_signal, sr = librosa.load(audio_path, sr=SAMPLE_RATE, mono=False)
        if audio_signal.ndim == 2:
            logging.info(f"Detected multi-channel audio ({audio_signal.shape[0]} channels), using first channel only.")
