...
# MODEL_PATH=/lustre/fsw/portfolios/convai/users/kevinhu/Checkpoints/Nemotron-VoiceChat-november/duplex-eartts-fp16-stt-22-november_stt_v2_fp32_boosteos/
...
# for audio_path in /lustre/fsw/portfolios/llmservice/users/kevinhu/data/elena_20251120/moshi_client_input_sf.wav /lustre/fsw/portfolios/llmservice/users/kevinhu/data/elena_20251120/moshi_client_input_maths.wav; do

# audio_path="/lustre/fsw/portfolios/llmservice/users/kevinhu/data/valentine_20251125/moshi_client_nemo_20251125_083423_combined_valentin_count3.wav"

# audio_name=$(basename ${audio_path} | sed 's/\.[^.]*$//')

# Get all wav files in a specified directory and decode them one by one

# wav_dir="/lustre/fsw/portfolios/llmservice/users/kevinhu/data/valentine_20251125"

# wav_dir="/lustre/fsw/portfolios/llmservice/users/kevinhu/data/ankita_20251125"
# wav_dir="/lustre/fsw/portfolios/llmservice/users/kevinhu/data/count_20251125"
wav_dir="/lustre/fsw/portfolios/llmservice/users/kevinhu/data/early_inter"

# Get list of all audio files (webm, wav, and m4a)
audio_files=()
shopt -s nullglob  # Prevent pattern from being literal if no matches
audio_files+=("${wav_dir}"/*.m4a)
audio_files+=("${wav_dir}"/*.webm)
audio_files+=("${wav_dir}"/*.wav)
shopt -u nullglob
...
if [[ "${audio_name}" != "moshi_client_nemo_20251204_101923_input" ]]; then
    continue
fi
...
python /lustre/fsw/portfolios/convai/users/kevinhu/S2S-Duplex-new-codebase/scripts/eartts/inference_streaming_realtime_niva.py \
    ...
    --output_stereo_audio="${RESULTS_DIR}/output_audio_${audio_name}_stereo.wav"
EOF
