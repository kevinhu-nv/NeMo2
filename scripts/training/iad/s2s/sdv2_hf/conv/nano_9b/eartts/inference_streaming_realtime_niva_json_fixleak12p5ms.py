...
class RealtimeStreamingInference:
    ...
    def _update_audio_buffer(self, audio_buffer, buffer_fill_level, new_audio, buffer_size_samples, overlap_samples=0):
        """
        ...
        Parameters:
            ...
            overlap_samples (int): Number of samples to remove from the end of the buffer before adding new audio.
        ...
        if buffer_fill_level + overlap_samples < buffer_size_samples and remaining.shape[1] > 0:
            ...
            if warmup_take > 0:
                ...
                buffer_fill_level += warmup_take - overlap_samples
        ...
        if remaining.shape[1] > 0:
            if remaining.shape[1] >= buffer_size_samples:
                ...
                audio_buffer = torch.cat([
                    audio_buffer[:, remaining.shape[1] - overlap_samples: -overlap_samples:],
                    remaining
                ], dim=1)
            buffer_fill_level = buffer_size_samples - overlap_samples
        current_buffer = audio_buffer if buffer_fill_level + overlap_samples == buffer_size_samples else audio_buffer[:, :buffer_fill_level]
        return audio_buffer, buffer_fill_level, current_buffer
    ...
    def inference_realtime_streaming(self, audio_path: str, num_frames_per_inference: int = None, explore_padding: bool = False, request_id: Optional[str] = None):
        ...
        buffer_size_samples = buffer_size_frames * FRAME_SIZE_SAMPLES + EXTRA_BUFFER_SAMPLES
        ...
        buffer_samples_per_step = FRAME_SIZE_SAMPLES * num_frames_per_inference + EXTRA_BUFFER_SAMPLES
        ...
        while frame_idx < total_frames:
            ...
            # Remove the overlapping portion (EXTRA_BUFFER_SAMPLES) before adding new audio
            audio_buffer, buffer_fill_level, current_buffer = self._update_audio_buffer(
                ...
                overlap_samples=EXTRA_BUFFER_SAMPLES
            )
