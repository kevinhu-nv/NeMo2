...
def add_noise_to_signal(signal, noise, snr_db):
    ...
    adjusted_noise = noise * (desired_noise_rms / noise_rms)
    repeat_count = int(np.ceil(len(signal) / len(adjusted_noise)))
    adjusted_noise = np.tile(adjusted_noise, repeat_count)[:len(signal)]
    noisy_signal = signal + adjusted_noise
    return noisy_signal            


def create_shar_from_manifest(manifest, out_shar_dir, noise_dir=None, snr_choices=[10, 30], num_shard=10, dataset_name='squadv2', turn_silence_sec=0.32):
    ...
    if noise_dir:
        ...
        if not noise_files:
            raise FileNotFoundError(f"No noise files found in directory: {noise_dir}")
    ...
    for i, line in tqdm(enumerate(in_manifest)):
        ...
        if convs[1]["transcript"] != 'I could not find the answer in the audio.':
            try:
                ...
                user_audio = user_recording.load_audio()

                if noise_dir:
                    noise_file = random.choice(noise_files)
                    noise_audio, _ = librosa.load(noise_file, sr=user_recording.sampling_rate)
                    snr_db = random.choice(snr_choices)
                    noisy_user_audio = add_noise_to_signal(user_audio[0], noise_audio, snr_db)
                    user_stream = BytesIO()
                    save_audio(dest=user_stream, src=noisy_user_audio, sampling_rate=user_recording.sampling_rate, format="wav")
                    user_stream.seek(0)
                    noisy_user_recording = Recording.from_bytes(user_stream.getvalue(), sampling_rate=user_recording.sampling_rate)
                    user_recordings.append(noisy_user_recording)
                else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing noise files (optional)"
    )
    parser.add_argument(
        '--num_shard',
        type=int,
