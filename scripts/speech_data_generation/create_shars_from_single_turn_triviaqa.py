    ...
    with tarfile.open(tar_path, "r") as tar:
        with tarfile.open(output_tar_path, "w") as out_tar:
            for member in tar.getmembers():
                ...
                # id_prefix = '_'.join(member_id.split('_')[:-1])
...
def create_single_turn_jsonl(input_manifest, target_tar_dir, source_tar_dir, out_dir):
    ...
    manifest_index = Path(input_manifest).stem.split('.')[-1]
    ...
    with open(input_manifest, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if 'dialog_id' in record:
                entries[record['dialog_id']] = record
            elif 'id' in record: 
    ...
    with gzip.open(jsonl_path, 'wt', encoding='utf-8') as gz_file:
        for turn_id in tqdm(ordered_ids, desc=f"Processing {manifest_index}"):
            ...
            user_text = entry['conversations'][0]['value'] if entry['conversations'][0]['speaker'] == 'user' else entry['conversations'][1]['value']
            agent_text = entry['conversations'][1]['value'] if entry['conversations'][1]['speaker'] == 'agent' else entry['conversations'][0]['value']
...
def main():
    ...
    manifest_file = Path(args.input_manifest_dir) / f"manifest.{args.shard_index:06d}.jsonl"
