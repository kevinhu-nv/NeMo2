#!/usr/bin/env python3
"""
TTS script using Tortoise TTS model.
Tortoise is a popular, high-quality TTS model available on GitHub.
"""
...
def get_wav_files_and_speakers(wav_dir):
    ...
    # Tortoise TTS voice presets
    available_voices = [
        "angie", "daniel", "emma", "geralt", "halle", "jlaw", "lj", "mol", "pat", "william"
    ]
...
def setup_tortoise():
    """Setup Tortoise TTS model."""
    try:
        # Import tortoise after installation
        ...
        print("Tortoise TTS not found. Please install it first:")
...
def generate_tts_audio(tts, text, voice, output_path):
    """Generate TTS audio using Tortoise TTS."""
    try:
        # Generate audio
        gen = tts.tts_with_preset(text, voice_samples=None, conditioning_latents=None, 
                                 preset="fast", use_deterministic_seed=42)
...
def main():
    parser = argparse.ArgumentParser(description="Generate TTS audio using Tortoise TTS with transcribed text from wav files")
