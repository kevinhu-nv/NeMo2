...
val_data=/lustre/fsw/portfolios/llmservice/projects/llmservice_nemo_speechlm/data/duplex/blackwell_test/
...
id=$(ls ${gen_wav_dir}/S1_A1*.gen.wav | grep -vE '_dup[01]\.gen\.wav' | awk -F'/' '{print $NF}' | grep -oE '^[^_]+_[^_]+_[^_]+_[0-9]+\.[0-9]+' | shuf -n 1)
# id=$(ls ${gen_wav_dir}/*.gen.wav | awk -F'/' '{print $NF}' | grep -oE '^[0-9]+' | shuf -n 1)
