...
# for subdir_name in en0_by_whisper en1_by_whisper; do
for subdir_name in en0_from_captions en1_by_whisper; do
  OUT_DIR="$BASE_OUT_DIR/YODAS_${subdir_name}"
  ...
  for m in $(seq 0 $MAX_MANIFEST_NUM); do
    MANIFEST="s3://YODAS/${subdir_name}/sharded_manifests/manifest_${m}.json"
  ...
  for i in $(seq 0 $MAX_TAR_NUM); do
    TAR="s3://YODAS/${subdir_name}/audio_${i}.tar"
