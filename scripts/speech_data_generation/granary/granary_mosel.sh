...
for subdir_name in en/tarred_train; do
  OUT_DIR="$BASE_OUT_DIR/MOSEL_${subdir_name}"
  ...
  for m in $(seq 0 $MAX_MANIFEST_NUM); do
    MANIFEST="s3://MOSEL/${subdir_name}/sharded_manifests/manifest_${m}.json"
  ...
  for i in $(seq 0 $MAX_TAR_NUM); do
    TAR="s3://MOSEL/${subdir_name}/audio_${i}.tar"
