...
for subdir_name in IHM-ASR/tarred; do
  OUT_DIR="$BASE_OUT_DIR/AMI_${subdir_name}"
  ...
  for m in $(seq 0 $MAX_MANIFEST_NUM); do
    MANIFEST="s3://AMI/${subdir_name}/sharded_manifests/manifest_${m}.json"
  ...
  for i in $(seq 0 $MAX_TAR_NUM); do
    TAR="s3://AMI/${subdir_name}/audio_${i}.tar"
