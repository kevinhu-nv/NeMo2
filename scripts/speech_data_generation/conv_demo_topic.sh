...
val_data=/lustre/fsw/portfolios/llmservice/users/kevinhu/duplex/topic_v2/Meta-Llama-3.1-70B-Instruct/shar_duplex/manifest_000050
...
id=$(ls ${gen_wav_dir}/topic_*_0.gen.wav 2>/dev/null | awk -F'/' '{print $NF}' | awk -F'_' '{print $2}' | shuf -n 1)
# id=$(ls ${gen_wav_dir}/*_0.gen.wav | awk -F'/' '{print $NF}' | grep -oE '^[0-9]+' | shuf -n 1)
# id=783
echo "Selected ID: $id"

for file in ${val_data}/cuts.*.jsonl.gz; do
    if zcat "$file" | grep -E -q "\"id\": \"${id}_"; then
        index=$(echo "$file" | grep -oE '[0-9]+' | tail -1)
        echo "Match found in: $file"

        zcat "$file" | grep "\"id\": \"${id}_" | jq

        # tar -xvf "${val_data}/recording.${index}.tar" -C ${out_dir} "${id}_0.flac"
        tar -xvf "${val_data}/recording.${index}.tar" -C ${out_dir} "topic_${id}_0.flac"
    fi
done

# Create stereo wav
# gen_wav=${gen_wav_dir}/${id}_0.gen.wav
gen_wav=${gen_wav_dir}/topic_${id}_0.gen.wav
set -x
# python /home/kevinhu/s2s/duplex/create_stereo.py --file1 ${out_dir}/${id}_0.flac --file2 ${gen_wav} --output_file ${gen_wav}.stereo.wav
python /home/kevinhu/s2s/duplex/create_stereo.py --file1 ${out_dir}/topic_${id}_0.flac --file2 ${gen_wav} --output_file ${gen_wav}.stereo.wav
set +x