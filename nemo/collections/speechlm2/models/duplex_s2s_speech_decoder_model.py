...
class DuplexS2SSpeechDecoderModel(LightningModule, HFHubMixin):
    ...
    def prepare_inputs(self, batch: dict):
        ...
        if self.cfg.get("debug", False):
            print(text_inputs[0]*(text_inputs[0]!=self.text_pad_id))
            first_nonpad_idx = (text_inputs[0] != self.text_pad_id).nonzero(as_tuple=True)[0][0].item() if (text_inputs[0] != self.text_pad_id).any() else None
            print("First non-pad token index in text_inputs[0]:", first_nonpad_idx)
            import pdb; pdb.set_trace()
